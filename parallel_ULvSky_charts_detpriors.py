...
def read_likelihood(lookup_file, line_no):
    ...
    try:
        ...
        print("Couldn't figure out how to do for:\n{} line: {}".format(lookup_file,line_no))
    return loglike
...
# 57697.590190464034
# 53451.99798753473
def likelihood_chart_wrapper(ipix, nside=8, Ts=nano12_Ts):
    import time
    start_time = time.time()
    outdir =  '/home/nima/nanograv/12p5yr_bwm/ULvSky_charts_crnmarg_detpriors_mapgamma/'
    ...
    bwm_log10_As = np.linspace(-17, -11, 60, endpoint=True)
    ...
    for ii, bwm_pol in enumerate(bwm_pols):
        ...
        if os.path.exists(loglike_chart_outfile):
            print("Found existing chart for pixel {} polarization {}".format(ipix, ii))
        else:
            ...
            for i, bwm_log10_A in enumerate(bwm_log10_As):
                for j, burst_epoch in enumerate(Ts):
                    ...
                    loglike_chart [i,j] = calculate_likelihood(bwm_theta, bwm_phi, bwm_log10_A,
                                         bwm_pol, burst_epoch,
                                         psrs,
                                         lookupdir = '/home/nima/nanograv/12p5yr_bwm/lookup_tables_crn_marg_det_RN_mapgamma/')
